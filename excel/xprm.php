...
$sheet->setCellValue('C2', 'DATOS PERMISO');
$sheet->mergeCells('C2:G2');
$sheet->setCellValue('H2', 'REVISION Y APROBACION');
$sheet->mergeCells('H2:J2');
$style = $sheet->getStyle('A2:J2');
$style->getFont()->setBold(true)->setSize(18);
$style->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('B7DEE8');

// Agregar titulos

$titulo = ['CEDULA', 'NOMBRE', 'AREA', 'FECHA SOLICITUD', 'TIPO PERMISO', 'TIEMPO PERMISO', 'DESCRIPCIÓN', 'APROBADO/RECHAZADO', 'NOMBRE', 'OBSERVACIÓN'];


$sheet->fromArray([$titulo], NULL, 'A3');
$style = $sheet->getStyle('A3:J3');
...
if ($datAll) {
    foreach ($datAll as $dat) {
            ...
            $filaDatos = [$dat['dper'], $dat['aper']." ".$dat['nper'], $dat['cper'],($dat['fecsol']), strtoupper($dat['tprm']), ($dat['fecini']) ." - " . ($dat['fecfin']), $dat['desprm'], (($dat['estprm'] == 3) ? 'Aprobado' : 'Rechazado') ,$dat['arev']." ".$dat['nrev'], $dat['obsprm']];
...
$range = 'A1:J'.($fila - 1); // Rango que cubre todos los datos
$sheet->getStyle($range)->applyFromArray($styleArray);
$sheet->getStyle($range)->applyFromArray($alignmentStyle);

// Ajustar la altura de las filas y el ancho de las columnas
foreach (range('A','J') as $columnID) $sheet->getColumnDimension($columnID)->setAutoSize(true);
